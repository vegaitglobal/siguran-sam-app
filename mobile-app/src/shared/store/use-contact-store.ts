...
export const addContact = (contact: Contact): Contact[] => {
  if (getContacts().length == maxContacts) {
    throw new ContactsFullError('Emergency contacts cannot exceed max capacity.');
  }
  if (useContactStore.getState().contacts.find((c) => c.number === contact.number)) {
    throw new DuplicateContactError(
      'Contact with number: ' + contact.number + ' and name ' + contact.name + ' already exists.'
    );
  }
